...
class ServiceController extends Controller
{
    ...
    public function newAction(Request $request, Utilities $utilities)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $resume = $utilities->resume($service->getContenu(), 200, '...', true);
            $service->setResume($resume);
            $em->persist($service);
            $em->flush();

            return $this->redirectToRoute('backend_service_show', array('slug' => $service->getSlug()));
        }
     ...
     * @Method({"GET", "POST"})
     */
    public function editAction(Request $request, Service $service, Utilities $utilities)
    {
        $deleteForm = $this->createDeleteForm($service);
